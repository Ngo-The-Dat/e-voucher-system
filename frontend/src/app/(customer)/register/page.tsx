...
import { Eye, EyeOff, CircleAlert, Gift, Shield, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function CustomerRegisterPage() {
  ...
  useEffect(() => {
    let timer: NodeJS.Timeout;
    if (resendAfter > 0) {
      timer = setInterval(() => setResendAfter((r) => r - 1), 1000);
    }
    return () => clearInterval(timer);
  }, [resendAfter]);
  ...
  const handleRequestOtp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const val = emailOrPhone.trim();
      const result = await customerAuthApi.requestRegistrationOtp({ email: val });
      setChallengeId(result.challenge_id);
      setCountdown(result.expires_in);
      setResendAfter(result.resend_after);
      setStep(2);
    } catch (regError: any) {
      setError(regError.message || "Yêu cầu gửi mã OTP thất bại.");
    } finally {
  ...
  const handleVerifyOtpAndRegister = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (otpCode.length !== 6) {
    ...
    try {
      ...
      // 1. Verify OTP
      await customerAuthApi.verifyRegistrationOtp({
        email: val,
      ...
      setError(regError.message || "Xác thực OTP hoặc đăng ký thất bại.");
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <>
              ...
              <div className="mb-8">
                ...
                <p className="text-gray-500">
                  Nhập mã 6 số được gửi đến <span className="font-medium text-gray-900">{emailOrPhone}</span>
                </p>
              </div>

              <form onSubmit={handleVerifyOtpAndRegister} className="space-y-6">
                <div>
                  <label htmlFor="otp-code" className="sr-only">Mã OTP</label>
                  <input
                    id="otp-code"
                    type="text"
                    value={otpCode}
                    ...
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl py-4 px-4 text-center text-2xl tracking-[0.5em] font-semibold text-gray-900 focus:outline-none focus:bg-white focus:border-[#0f2c59] focus:ring-1 focus:ring-[#0f2c59] transition-colors"
                    maxLength={6}
                    autoFocus
                  />
                  {countdown > 0 ? (
                    <p className="text-center text-sm text-gray-500 mt-4">
                      Mã có hiệu lực trong: <span className="font-semibold text-blue-600">{Math.floor(countdown / 60)}:{(countdown % 60).toString().padStart(2, "0")}</span>
                    </p>
                  ) : (
                ...
                <button
                  type="submit"
                  disabled={isSubmitting || otpCode.length !== 6 || countdown === 0}
                  className="w-full bg-[#0f2c59] text-white font-bold py-4 rounded-xl hover:bg-[#0f2c59]/90 transition-colors shadow-sm disabled:opacity-70 flex justify-center cursor-pointer"
                >
                  {isSubmitting ? "Đang xác thực..." : "Xác nhận và Đăng ký"}
                </button>
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-500">
                    ...
                    <button
                      ...
                      className="font-semibold text-[#0f2c59] hover:underline disabled:text-gray-400 disabled:no-underline ml-1"
                    >
