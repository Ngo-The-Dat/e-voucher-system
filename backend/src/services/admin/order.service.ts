...
export async function getOrders(filter: GetOrdersFilter = {}) {
  ...
  // 2. Thống kê theo trạng thái đơn hàng (cho các tab/stats)
  const statsQuery = `
    SELECT
      COUNT(*) as all_count,
...
export async function getOrderById(orderId: number) {
  ...
  // Lấy các mục hàng (order_items) kèm chương trình voucher và danh sách voucher phát hành (issued_vouchers)
  const itemsQuery = `
    SELECT 
      oi.order_item_id,
      oi.order_id,
